...
- GPU ready: uses CUDA if available
"""
...
# replace your current pykakasi block with this
try:
...
def jp_with_readings(text: str) -> Tuple[str, str]:
    """Return (hiragana, romaji) if pykakasi is available; otherwise ('','')."""
    if not _HAS_KAKASI:
        return "", ""
    hira = _KAKASI_hira.do(text)
    roma = _KAKASI_roma.do(text)
    # small tidy-ups
    roma = roma.replace("  ", " ").strip()
    return hira, roma
...
def transcribe_audio(
    ...
    device = "cuda" if compute_type in {"float16", "int8_float16", "int8"} else "cpu"
    if verbose:
        logging.info(f"Loading Whisper model='{model_size}' compute_type='{compute_type}'")
    ...
    if verbose:
        logging.info(f"Detected primary language: {getattr(info, 'language', None)} "
                     f"(prob {getattr(info, 'language_probability', None)})")
        logging.info(f"ASR segments: {len(segments)}")
    return segments
...
def diarize_speakers(
    ...
    if verbose:
        nspk = len(set(label for _, _, label in diarization.itertracks(yield_label=True)))
        logging.info(f"Diarization done. Unique speakers: {nspk}")
    return diarization
...
def parse_args():
    ...
    ap.add_argument("--model-size", default="medium", help="Whisper size: tiny/base/small/medium/large-v3")
