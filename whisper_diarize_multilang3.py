...
-----------------------------------------------------------------
- Inputs: an AUDIO file path (e.g., mp3/wav/flac)  <-- updated: audio-only
...
- TorchCodec removed; diarization always uses a preloaded {waveform, sample_rate} dict
  with torchaudio + FFmpeg fallback for decoding
"""
...
# --- torch/torchaudio for preload + device detection
import torch
import torchaudio

# --- FFmpeg fallback (robust MP3 handling on Windows)
import subprocess
import tempfile
import shutil
...
def _ffmpeg_decode_to_tmp_wav(src: str, dst: str, sr: int = 16000):
    # -vn ensures we ignore any accidental video streams; -ac 1 mono; -ar resample
    cmd = ["ffmpeg", "-y", "-i", src, "-vn", "-ac", "1", "-ar", str(sr), "-f", "wav", dst]
    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def load_audio_ta(path: str, target_sr: int = 16000) -> dict:
    """
    Preload audio as mono float32 @ target_sr for pyannote.
    Try torchaudio.load first (fast); if it fails (common for MP3 on Windows),
    fall back to FFmpeg CLI to decode to a temp WAV, then load with torchaudio.
    """
    try:
        ...
        if not _has_ffmpeg():
            raise RuntimeError(
                "torchaudio could not decode the file and ffmpeg is not installed. "
                "Install FFmpeg (e.g., `conda install -c conda-forge ffmpeg`) and ensure it is on PATH."
            )
        with tempfile.TemporaryDirectory() as td:
            tmp = os.path.join(td, "tmp.wav")
            _ffmpeg_decode_to_tmp_wav(path, tmp, sr=target_sr)
...
def diarize_speakers(
    ...
    """
    Diarize using pyannote. We ALWAYS use the preloaded-audio dict path
    to avoid TorchCodec and ensure Windows MP3 reliability.
    """
    if not hf_token:
    ...
    if verbose:
        logging.info("[Pyannote] using preloaded-audio (torchaudio + FFmpeg fallback)")
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Transcribe + diarize + per-segment language ID using Whisper & Pyannote. Adds study output."
    )
    ap.add_argument("input", help="Path to AUDIO file (mp3/wav/flac).")
    ap.add_argument(
        "--formats",
...
def main():
    ...
    # Diarize (Pyannote) — ALWAYS use preloaded audio dict (no TorchCodec)
    diar = diarize_speakers(
        audio_input=str(in_path),  # will be preloaded inside diarize_speakers()
